 ...
 * POSITION_TOLERANCE
 * ------------------
 * Maximum horizontal distance (in pixels) between text elements
 * for them to be considered aligned to the same column.
 *
 * Higher value  = more tolerant of alignment differences.
 * Lower value   = stricter column detection.
 */
const POSITION_TOLERANCE = 8;

/**
 * MIN_COLUMN_OCCURRENCES
 * ----------------------
 * Minimum number of text elements that must appear at approximately
 * the same horizontal position for that position to be recognized
 * as a valid table column.
 *
 * Higher value = fewer false-positive columns.
 * Lower value  = more columns may be detected.
 */
 ...
 * MIN_TABLE_ROWS
 * ---------------
 * Minimum number of rows required for a group of detected columns
 * to be considered a table.
 *
 * Higher value = stricter table detection.
 * Lower value  = smaller tables can be detected.
 */
 ...
 * CELL_JOIN_GAP
 * --------------
 * Maximum horizontal gap (in pixels) between text elements within
 * the same row for them to be joined into a single cell.
 *
 * Higher value = more text elements are merged together.
 * Lower value  = text is split into separate cells more easily.
 */
const CELL_JOIN_GAP = 3;

/**
 * MIN_COLUMN_GAP
 * --------------
 * Minimum horizontal distance (in pixels) required between two
 * detected column positions.
 *
 * Prevents multiple nearby positions from being treated as
 * separate columns.
 *
 * Higher value = fewer, more widely separated columns.
 * Lower value  = more closely spaced columns can be detected.
 */
 ...
 * MAX_COLUMN_COUNT
 * -----------------
 * Maximum number of columns that the table detector is allowed
 * to identify within a table region.
 *
 * Prevents abnormal PDF layouts or noisy text positions from
 * producing an excessive number of columns.
 */
 ...
 * TABLE_REGION_GAP
 * ----------------
 * Maximum vertical gap (in pixels) between consecutive lines for
 * them to be considered part of the same table region.
 *
 * Higher value = lines farther apart can belong to the same table.
 * Lower value  = table regions are split more easily.
 */
 ...
 * MIN_REGION_LINES
 * -----------------
 * Minimum number of text lines required for a group of lines
 * to qualify as a potential table region.
 *
 * Higher value = stricter table-region detection.
 * Lower value  = smaller regions can be considered tables.
 */
